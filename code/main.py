...
test_loader = DataLoader(dataset=test_dataset, batch_size=1, collate_fn=collate_fn, shuffle=False,
						 num_workers=NUM_WORKERS)
...
def run(task, fuse, encoder):
	...
	model = model_classes[fuse](encoder=encoder)
	...
	torch.save(model, os.path.join(PATH_OUTPUT, "{}_{}_{}.pth".format(task, encoder, fuse)))
	best_model = torch.load(os.path.join(PATH_OUTPUT, "{}_{}_{}.pth".format(task, encoder, fuse)))
...
results = []
for encoder in ["LSTM", "Transformer"]:
	for fuse in ['EFI', 'EFE', 'LF', 'LFpool']:
		result = [str(i) for i in run(task, fuse, encoder)]
		results.append([task, fuse, encoder]+result)

with open(os.path.join(PATH_OUTPUT,"output.txt"), 'w') as f:
	for res in results:
		f.write(', '.join(res))
		f.write('\n')